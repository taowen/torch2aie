...
constexpr int32_t kHeads = 5;   // BitNet: 5 Q heads per window (20 Q heads / 4 windows)
constexpr int32_t kContext = 16;
constexpr int32_t kHeadDim = 128;
constexpr int32_t kGqaRatio = 4;   // BitNet: 20 Q heads / 5 KV heads
constexpr int32_t kWeightDwords = kHeads * kContext / 2;  // 5*16/2 = 40
constexpr int32_t kVecLanes = 32;
constexpr int32_t kAccumLanes = kHeads * kHeadDim;    // 5*128 = 640
constexpr int32_t kStateDwords = kHeads * 2;           // 10
...
#ifdef BITNET_ENABLE_EDGE_ATTENTION_CYCLE_PROFILE
...
void bitnet_attention_bf16_init_accum(
    int32_t *accum,
    int32_t *state,
    int32_t accum_lanes,
    ...
    for (int32_t idx = 0; idx < kAccumLanes; idx += kVecLanes)
        chess_prepare_for_pipelining chess_loop_range(20, 20) {
        aie::store_v(accum_values + idx, zero_vec);
    }
    for (int32_t si = 0; si < kStateDwords; si++)
        chess_prepare_for_pipelining chess_loop_range(1, 16) {
        state_values[si] = 0.0f;
    }
...
void bitnet_attention_bf16_finish_accum(
    int32_t *accum,
    int32_t *state,
    int32_t *output,
