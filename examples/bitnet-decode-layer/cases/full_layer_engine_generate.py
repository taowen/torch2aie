"""Generate MLIR-AIE for the BitNet full-layer fused engine."""
...
from compact_dataflow import (
    ...
    _phase_trace_marker,
    compact_phase_trace,
    bf16_weight_column_memtile,
)
from physical_contract import validate_bf16_bitnet_down_full_layer_ownership
...
from cases.full_layer_engine_reference import (
    CASE_NAME,
    COLUMN_WEIGHT_BF16,
    COLUMN_WEIGHT_BF16_RAW,
    DEFAULT_SCHEDULE,
    HIDDEN_DWORDS,
    OUTPUT_DWORDS,
    PATCH_WEIGHT_BF16,
    PATCH_WEIGHT_BF16_RAW,
    QK_ROPE_DWORDS,
    RMS_NORM_DWORDS,
    AUX_DWORDS,
...
MAIN16_KERNEL_OBJECT = "main_projection_bitnet_fast.o"
MAIN16_LAYER_SCHEDULER = "bitnet_main16_layer_scheduler"
...
MAIN_RECORD_PING_ADDR = 0xA800
MAIN_CONTROL_ADDR = 0xA900
MAIN_RECORD_PONG_ADDR = 0xAA00
...
MAIN16_BUFFERS = (
    ...
    Main16Buffer(
        ...
        CHUNK_BF16_RAW * 2,
        MAIN16_ALL_PHASES,
        "DMA1 BF16 raw weight ping",
        f"memref<{CHUNK_BF16_RAW}xbf16>",
    ),
    Main16Buffer(
        "wt_pong",
        0x6800,
...
def _runtime_sequence(schedule: DecodeSchedule) -> str:
    lines = [
        ...
        f"%weights: memref<{TOTAL_WEIGHT_AND_AUX_I32_RAW}xi32>, "
    ...
    chunk_pair_bytes = ROWS_PER_PATCH * CHUNK_BF16_RAW * 2
    for group, column in enumerate(MAIN_COLUMNS):
        column_base = AUX_DWORDS * 4 + group * COLUMN_WEIGHT_BF16_RAW * 2
        for patch, bd_ids in enumerate(WEIGHT_PATCH_BD_IDS):
            patch_base = column_base + patch * PATCH_WEIGHT_BF16_RAW * 2
            for span_idx, (chunk_base, chunk_count) in enumerate(weight_spans):
                ...
                dwords = chunk_count * ROWS_PER_PATCH * CHUNK_BF16_RAW // 2
                lines.extend(
                    (
                        npu_writebd(
                            column,
...
{lock_pair(tile, "carrier", 4)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      scf.for %block = %c0 to %blocks step %c1 {{
        ...
        func.call @bitnet_attention_bf16_make_carrier_masked(%{tile}_q, %{tile}_k, %{tile}_carrier, %window_i32, %block_i32, %blocks_i32, %tail_tokens_i32, %q_dwords_i32, %k_dwords_i32, %carrier_dwords_i32)
...
{lock_pair(tile, "output", 4)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      func.call @bitnet_attention_bf16_init_accum(%{tile}_accum, %{tile}_state, %accum_lanes_i32, %state_dwords_i32)
        : (memref<{ACCUM_LANES}xi32>, memref<{SCALAR_DWORDS}xi32>, i32, i32) -> ()
      scf.for %block = %c0 to %blocks step %c1 {{
        %block_i32 = arith.index_cast %block : index to i32
        aie.use_lock(%{tile}_v_full, AcquireGreaterEqual, 1)
        aie.use_lock(%{tile}_carrier_full, AcquireGreaterEqual, 1)
        func.call @bitnet_attention_bf16_accum_block(%{tile}_v, %{tile}_carrier, %{tile}_accum, %{tile}_state, %block_i32, %v_dwords_i32, %carrier_dwords_i32, %accum_lanes_i32, %state_dwords_i32)
      ...
      func.call @bitnet_attention_bf16_finish_accum(%{tile}_accum, %{tile}_state, %{tile}_output, %out_dwords_i32, %accum_lanes_i32, %state_dwords_i32)
...
{lock_pair("post", "qk_rope_side", 13)}
    ...
    %post_core = aie.core(%post) {{
      ...
      scf.for %record = %c0 to %qkv_records step %c1 {{
        ...
        func.call @bitnet_postprocess_absorb_qkv_payload_record(%post_qkv_payload, %post_q_body, %post_k_body, %post_v_body, %record_i32)
      ...
      func.call @bitnet_postprocess_body_payload(%post_q_body, %post_k_body, %post_v_body, %post_qk_rope_side, %post_q_payload, %post_current_k, %post_current_v, %post_current_token, %q_dwords_i32, %current_dwords_i32)
...
def resource_manifest_for_case(
    ...
    return ResourceManifest(
        schema="bitnet-layer-resource-manifest-v1",
        case=case_name,
        tiles=tuple(
        ...
        bds=(
            ...
            *_main_record_bds(tile, row, phase_trace),
            BDResource(
                ...
                channel=1,
                direction="S2MM",
                length=CHUNK_BF16_RAW,
                buffer=f"{tile}_wt_ping",
                phases=_all_phases(),
                semantic="main16 DMA1 BF16 raw weight ping",
            ...
            BDResource(
                ...
                phases=_all_phases(),
                semantic="main16 DMA1 BF16 raw weight pong",
...
def _main16_layer_scheduler_call(tile: str, group: int, row: int, phase_limit: int) -> str:
    ...
    return f"""
      ...
      func.call @{MAIN16_LAYER_SCHEDULER}(%{tile}_wt_ping, %{tile}_wt_pong, %{tile}_chunk_ping, %{tile}_chunk_pong, {_record_ping_name(tile)}, {_record_pong_name(tile)}, %group_i32, %row_i32, %m_i32, %main16_phase_limit_i32)
        : (memref<{CHUNK_BF16_RAW}xbf16>, memref<{CHUNK_BF16_RAW}xbf16>, memref<{MAIN_CHUNK_DWORDS}xi32>, memref<{MAIN_CHUNK_DWORDS}xi32>, {record_type}, {record_type}, i32, i32, i32, i32) -> ()"""
...
{_main16_layer_scheduler_call(tile, group, row, MAIN16_PHASE_LIMIT_QKV)}
    ...
    ^wt_ping:
      ...
      aie.dma_bd(%{tile}_wt_ping : memref<{CHUNK_BF16_RAW}xbf16>, 0, {CHUNK_BF16_RAW}) {{bd_id = {MAIN_WEIGHT_BDS[0]} : i32, next_bd_id = {MAIN_WEIGHT_BDS[1]} : i32}}
      aie.use_lock(%{tile}_wt_full, Release, 1)
      aie.next_bd ^wt_pong
    ^wt_pong:
      aie.use_lock(%{tile}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_wt_pong : memref<{CHUNK_BF16_RAW}xbf16>, 0, {CHUNK_BF16_RAW}) {{bd_id = {MAIN_WEIGHT_BDS[1]} : i32, next_bd_id = {MAIN_WEIGHT_BDS[0]} : i32}}
      aie.use_lock(%{tile}_wt_full, Release, 1)
      aie.next_bd ^wt_ping
...
def generate_mlir(schedule: DecodeSchedule = DEFAULT_SCHEDULE) -> str:
    ...
    for group in range(len(MAIN_COLUMNS)):
        blocks.append(bf16_weight_column_memtile(group, QKV_BODY_PHASE_TRACE))
        for row in range(len(MAIN_ROWS)):
...
{chr(10).join(flows)}
    ...
    func.func private @bitnet_postprocess_absorb_qkv_payload_record(memref<{COMPACT_PACKET_DWORDS - 1}xi32>, memref<{Q_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, i32) attributes {{link_with = "{experiment_dir}/postprocess_qkv.o"}}
    func.func private @bitnet_postprocess_body_payload(memref<{Q_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, memref<{QK_ROPE_DWORDS}xi32>, memref<{Q_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, memref<{CURRENT_DWORDS}xi32>, memref<1xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/postprocess_qkv.o"}}
    ...
    func.func private @bitnet_attention_bf16_make_carrier_masked(memref<{WINDOW_DWORDS}xi32>, memref<{K_WINDOW_DWORDS}xi32>, memref<{SHAPE_CARRIER_DWORDS}xi32>, i32, i32, i32, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/edge_attention.o"}}
    func.func private @bitnet_attention_bf16_init_accum(memref<{ACCUM_LANES}xi32>, memref<{SCALAR_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_attention.o"}}
    func.func private @bitnet_attention_bf16_accum_block(memref<{V_WINDOW_DWORDS}xi32>, memref<{SHAPE_CARRIER_DWORDS}xi32>, memref<{ACCUM_LANES}xi32>, memref<{SCALAR_DWORDS}xi32>, i32, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/edge_attention.o"}}
    func.func private @bitnet_attention_bf16_finish_accum(memref<{ACCUM_LANES}xi32>, memref<{SCALAR_DWORDS}xi32>, memref<{ATTENTION_OUTPUT_DWORDS}xi32>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/edge_attention.o"}}
    func.func private @{MAIN16_LAYER_SCHEDULER}(memref<{CHUNK_BF16_RAW}xbf16>, memref<{CHUNK_BF16_RAW}xbf16>, memref<{MAIN_CHUNK_DWORDS}xi32>, memref<{MAIN_CHUNK_DWORDS}xi32>, memref<{MAIN_RECORD_PINGPONG_DWORDS}xi32>, memref<{MAIN_RECORD_PINGPONG_DWORDS}xi32>, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/{MAIN16_KERNEL_OBJECT}"}}
...
def validate_generated_mlir(mlir: str, schedule: DecodeSchedule = DEFAULT_SCHEDULE) -> list[str]:
    ...
    required = (
        ...
        "bitnet_attention_bf16_make_carrier_masked",
        "bitnet_attention_bf16_finish_accum",
        ...
        f"memref<{TOTAL_WEIGHT_AND_AUX_I32_RAW}xi32>",
        ...
        "aie.dma_start(S2MM, 4, ^patch0_bf16_ping, ^patch1_start)",
        "aie.dma_start(S2MM, 5, ^patch1_bf16_ping, ^wt_row0_start)",
    ...
    errors.extend(require_count(CASE_NAME, "bitnet main16 layer scheduler calls", mlir.count(f"func.call @{MAIN16_LAYER_SCHEDULER}"), main_tile_count))
    ...
    errors.extend(require_count(CASE_NAME, "old q emit calls in MLIR", mlir.count("func.call @q4nx_emit_q_accum_body_record"), 0))
    errors.extend(require_count(CASE_NAME, "old k emit calls in MLIR", mlir.count("func.call @q4nx_emit_k_accum_body_record"), 0))
    errors.extend(require_count(CASE_NAME, "old v emit calls in MLIR", mlir.count("func.call @q4nx_emit_v_accum_body_record"), 0))
    ...
    errors.extend(require_count(CASE_NAME, "qwen3 compact record absorb", mlir.count("bitnet_postprocess_absorb_qkv_payload_record"), 2))
    errors.extend(require_count(CASE_NAME, "bitnet body postprocess", mlir.count("bitnet_postprocess_body_payload"), 2))
    errors.extend(require_count(CASE_NAME, "bitnet_attention_bf16_make_carrier_masked", mlir.count("bitnet_attention_bf16_make_carrier_masked"), 5))
    errors.extend(require_count(CASE_NAME, "bitnet_attention_bf16_init_accum", mlir.count("bitnet_attention_bf16_init_accum"), 5))
    errors.extend(require_count(CASE_NAME, "bitnet_attention_bf16_accum_block", mlir.count("bitnet_attention_bf16_accum_block"), 5))
    errors.extend(require_count(CASE_NAME, "bitnet_attention_bf16_finish_accum", mlir.count("bitnet_attention_bf16_finish_accum"), 5))
    errors.extend(require_count(CASE_NAME, "old o emit calls in MLIR", mlir.count("func.call @q4nx_emit_o_block_accum_record"), 0))
    ...
    errors.extend(validate_bf16_bitnet_down_full_layer_ownership(CASE_NAME, mlir))
