...
static volatile int32_t g_weight_stream_sink;

__attribute__((always_inline)) static inline void touch_weight_chunk(
    bfloat16 *packed_chunk) {
  volatile int32_t *words =
      reinterpret_cast<volatile int32_t *>(packed_chunk);
  g_weight_stream_sink ^= words[0];
  g_weight_stream_sink ^= words[kChunkDwords - 1];
}
...
static void run_weight_stream_phase(bfloat16 *wt_ping, bfloat16 *wt_pong,
                                    int32_t *record_ping,
...
void q4nx_main16_layer_scheduler(bfloat16 *wt_ping, bfloat16 *wt_pong,
  ...
  if (phase_limit > qwen3::kQPhase) {
    run_weight_stream_phase(wt_ping, wt_pong, record_ping, record_pong,
